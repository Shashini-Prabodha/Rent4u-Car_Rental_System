...
public class CustomerController {
    ...
    public boolean saveFile(@RequestPart("slip") MultipartFile slipFile) {

        try {
            String projectPath = new File(this.getClass().getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile().getParentFile().getAbsolutePath();
            File uploadsDir = new File(projectPath + "/customer/slip");
            uploadsDir.mkdir();
            slipFile.transferTo(new File(uploadsDir.getAbsolutePath() + "/" + slipFile.getOriginalFilename()));
            return true;
